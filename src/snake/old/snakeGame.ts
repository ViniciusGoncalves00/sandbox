...
import type { Context3D } from "../base/viewport";
...
export class SnakeGame implements Loopable {
    ...
    private viewport: Context3D;
    private updateRate: number;
    private accumulator: number = 0;

    public constructor(viewport: Context3D, snakeGameParams: SnakeGameParameters, player: Player) {
        ...
        const head: Vector2Int = {
            ...
            y: Math.floor(snakeGameParams.board.height / 2)
        }
        const tail: Vector2Int = {x: head.x, y: head.y - 1};
    ...
    public updateSnake(direction: Vector2Int): void {
        ...
        const newHead: Vector2Int = { x: head.x + direction.x, y: head.y + direction.y};
