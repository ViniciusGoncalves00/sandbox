import { Color, CylinderGeometry, ConeGeometry, Mesh, MeshBasicMaterial, Object3D, Vector3, DoubleSide, PlaneGeometry, EdgesGeometry, LineSegments, LineBasicMaterial, CircleGeometry, BoxGeometry } from "three";
...
export interface HandlerObject {
    ...
    worldRightScale: Object3D;
    worldUpwardScale: Object3D;
    worldForwardScale: Object3D;
    localRightScale: Object3D;
    localUpwardScale: Object3D;
    localForwardScale: Object3D;
}
...
export const DEFAULT_HANDLER_PARAMETERS: Required<HandlerParameters> = {
    handler: {
        worldRightTranslate: createTranslateHandler(new Color("red")).rotateZ(-Math.PI / 2).translateY(0.25 / 2),
        worldUpwardTranslate: createTranslateHandler(new Color("green")).translateY(0.25 / 2),
        worldForwardTranslate: createTranslateHandler(new Color("blue")).rotateX(Math.PI / 2).translateY(0.25 / 2),
        localRightTranslate: createTranslateHandler(new Color("cyan")).rotateZ(-Math.PI / 2).translateY(0.25 / 2),
        localUpwardTranslate: createTranslateHandler(new Color("magenta")).translateY(0.25 / 2),
        localForwardTranslate: createTranslateHandler(new Color("yellow")).rotateX(Math.PI / 2).translateY(0.25 / 2),
        ...
        worldRightScale: createScaleHandler(new Color("red")).rotateZ(-Math.PI / 2).translateY(0.25 / 2),
        worldUpwardScale: createScaleHandler(new Color("green")).translateY(0.25 / 2),
        worldForwardScale: createScaleHandler(new Color("blue")).rotateX(Math.PI / 2).translateY(0.25 / 2),
        localRightScale: createScaleHandler(new Color("cyan")).rotateZ(-Math.PI / 2).translateY(0.25 / 2),
        localUpwardScale: createScaleHandler(new Color("magenta")).translateY(0.25 / 2),
        localForwardScale: createScaleHandler(new Color("yellow")).rotateX(Math.PI / 2).translateY(0.25 / 2),
    }
};
...
function createScaleHandler(
    ...
    const head = new Mesh(
        new BoxGeometry(headLength, headLength, headLength),
        material
    );
    head.position.y = bodyLength;
